...
eigenval_1,eigenvec_1=np.linalg.eigh(np.dot(A,np.transpose(A)))      # calculating eigenvalues and eigen vectors of A*(A^T)
eigenval_2,eigenvec_2=np.linalg.eigh(np.dot(np.transpose(A),A))      #eigenvalues and eigen vectors of (A^T)*A
...
S2=np.dot(np.dot(np.transpose(U2),A),V2)
